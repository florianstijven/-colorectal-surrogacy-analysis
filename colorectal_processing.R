...
load(file = "sensitivity_analysis_results.RData")
...
path = "Figures/No Conditional Independence/"
...
sens_results = sens_results_no_cond_ind %>% mutate(icc = 1 - exp(-2*minfo))

# The proportion diseased is computed in both treatment groups. This is
# essentially the proportion of individuals that experience the surrogate event
# (progression) before the true event (death).
sens_results %>% summarise(prop_diseased_0 = mean(prop_always + prop_harmed),
                           prop_diseased_1 = mean(prop_always + prop_protected))
# prop_diseased_0 prop_diseased_1
# 0.6944372       0.6663787
